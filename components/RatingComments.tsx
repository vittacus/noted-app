        ...
        <div className="mt-2 space-y-2">
          ...
          {comments.map((c) => (
            <div key={c.id} className="flex gap-2">
              <span className="text-xs font-semibold text-[#F5A623] shrink-0 mt-0.5">
                {c.user?.username ?? "?"}
              </span>
          ...
          {isLoggedIn ? (
            <form onSubmit={handleSubmit} className="flex gap-2 mt-2">
              <input
                ...
                maxLength={500}
                className="flex-1 text-xs bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-slate-200 placeholder-white/20 focus:outline-none focus:ring-1 focus:ring-[#F5A623]/50"
              />
              <button type="submit" disabled={!text.trim() || submitting}
                className="w-8 h-8 rounded-xl bg-[#F5A623]/10 border border-[#F5A623]/20 flex items-center justify-center hover:bg-[#F5A623]/20 transition-colors disabled:opacity-40">
                <Send size={12} className="text-[#F5A623]" />
              </button>
            </form>
          ) : (
            <p className="text-xs text-white/28 mt-1">
              <a href="/auth/login" className="text-[#F5A623] hover:underline">Sign in</a> to comment
            </p>
          )}
        </div>
      )}
