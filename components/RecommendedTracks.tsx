      ...
      <div className="mb-8">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="shrink-0 w-[120px] bg-[#161616] rounded-2xl border border-white/8 overflow-hidden animate-pulse">
        ...
        <div className="space-y-0.5">
          {tracks.map((track) => (
            ...
            >
              ...
              <span className="shrink-0 text-xs font-semibold text-[#117ACA] px-2 py-1 rounded-lg bg-[#117ACA]/10 border border-[#117ACA]/20 opacity-40 group-hover:opacity-100 transition-opacity">
                Rate
              </span>
            </button>
          ))}
        </div>
        <Link href="/search" className="block text-xs text-[#117ACA] hover:underline mt-2 px-2">
          See more →
        </Link>
        {!onRate && ratingTrack && (
          <RatingModal
          ...
          >
            <div className="relative w-full aspect-square">
              {track.album?.images?.[0] ? (
                ...
                <div className="absolute inset-0" style={{ backgroundColor: "#212121" }} />
              )}
              <div
