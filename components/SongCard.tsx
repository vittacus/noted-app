    ...
    <div className="flex items-center gap-3 bg-white rounded-2xl p-3 shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
      ...
      <div className="relative shrink-0 rounded-xl overflow-hidden bg-slate-100" style={{ width: 52, height: 52 }}>
        {song.album_art_url ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-[#F5A623]/10 to-[#F5A623]/20" />
        )}
      </div>
      ...
      {!compact && (
        <div className="flex flex-col gap-1.5 shrink-0">
          <button
            onClick={() => onRate?.(song)}
            className="w-8 h-8 rounded-full bg-[#F5A623]/5 hover:bg-[#F5A623]/10 flex items-center justify-center transition-colors"
          >
