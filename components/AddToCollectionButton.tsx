  ...
  return (
    <>
      <button
        ...
        className="w-full h-12 rounded-2xl border border-white/10 text-white/50 text-sm font-semibold hover:bg-white/5 transition-colors flex items-center justify-center gap-2"
      >
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-[#161616] rounded-t-3xl border-t border-white/8 p-5 max-h-[70vh] flex flex-col">
            ...
            <div className="flex gap-2 mb-4">
              <input
                ...
                className="flex-1 px-3 py-2.5 rounded-xl border border-white/10 bg-white/5 text-sm text-slate-100 placeholder-white/20 focus:outline-none focus:ring-2 focus:ring-[#117ACA]/50"
              />
              <button
                onClick={createCollection}
                disabled={!newName.trim() || creating}
                className="px-3.5 py-2.5 rounded-xl bg-[#117ACA]/20 text-[#117ACA] text-sm font-semibold hover:bg-[#117ACA]/30 transition-colors disabled:opacity-40 flex items-center gap-1.5"
              >
                <Plus size={14} /> Create
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-2">
              {loading && <p className="text-xs text-white/38 text-center py-4">Loading…</p>}
              {!loading && collections.length === 0 && (
                <p className="text-xs text-white/38 text-center py-4">No collections yet. Create one above.</p>
              )}
              {collections.map((col) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl border transition-all text-left ${
                      inCollection
                        ? "border-[#117ACA]/40 bg-[#117ACA]/10"
                        : "border-white/8 bg-white/5 hover:border-white/10"
                    }`}
                  >
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                      inCollection ? "border-[#117ACA] bg-[#117ACA]" : "border-white/12"
                    }`}>
