    ...
    <div className="page-enter">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-white/50">{songs.length} song{songs.length !== 1 ? "s" : ""}</p>
        </div>
      </div>
      ...
      {songs.length === 0 && (
        <div className="text-center py-16">
          ...
          <Link href="/library" className="text-[#F5A623] text-sm font-semibold hover:underline mt-3 block">Browse library →</Link>
        </div>
      )}

      <div className="space-y-2 mb-6">
        {songs.map((entry, i) => (
          <div key={entry.id} className="flex items-center gap-3 bg-[#111111] rounded-2xl p-3 border border-white/8">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm text-slate-100 truncate">
                {entry.rating_id
                  ? <Link href={`/song/${entry.rating_id}`} className="hover:text-[#F5A623] transition-colors">{entry.song.title}</Link>
                  : entry.song.title}
              </p>
            ...
            {isOwner && (
              <button onClick={() => removeFromCollection(entry.id)} className="ml-1 w-7 h-7 rounded-full bg-white/5 flex items-center justify-center hover:bg-rose-500/20 transition-colors shrink-0">
                <Trash2 size={12} className="text-white/50 hover:text-rose-400" />
              </button>
            )}
          </div>
        ))}
      ...
      <div className="relative">
        ...
        {tooltipVisible && (
          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-[#111111] border border-white/10 rounded-xl text-xs text-white/50 whitespace-nowrap shadow-lg">
