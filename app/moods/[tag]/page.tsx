  ...
  return (
    <div className="page-enter" onClick={() => { if (openSwipeId) setOpenSwipeId(null); }}>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button onClick={openPicker} className="w-9 h-9 rounded-full bg-[#F5A623]/10 border border-[#F5A623]/20 flex items-center justify-center hover:bg-[#F5A623]/20 transition-colors shrink-0">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 mb-4 -mx-4 px-4">
              <button onClick={() => setArtistFilter("all")}
                className={`shrink-0 px-3 py-1 rounded-full text-xs font-semibold border transition-all ${
                  artistFilter === "all"
                    ? "bg-[#F5A623]/20 border-[#F5A623]/60 text-[#F5A623]"
      ...
      {processed.length === 0 && songs.length > 0 && (
        <div className="text-center py-10">
          ...
          <button onClick={() => { setScoreFilter("all"); setArtistFilter("all"); }}
            className="text-[#F5A623] text-xs font-semibold mt-2 hover:underline">Clear filters →</button>
        </div>
      )}
        ...
        {displayed.map((r: any, i) => {
          ...
          return (
            <div key={r.id}
              ...
              <div
                className={`relative z-10 bg-[#111111] border border-white/8 rounded-2xl transition-transform duration-200 ease-out group ${
      ...
      <div className="relative">
        ...
        {tooltipVisible && (
          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-[#111111] border border-white/10 rounded-xl text-xs text-white/50 whitespace-nowrap shadow-lg pointer-events-none">
      ...
      {showPicker && (
        <div className="fixed inset-0 z-[100] flex flex-col justify-end" style={{ isolation: "isolate" }}>
          ...
          <div className="relative w-full bg-[#000000] rounded-t-3xl flex flex-col"
            ...
            <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2 min-h-0">
              {pickerLoading && <div className="flex justify-center py-10"><div className="w-5 h-5 border-2 border-[#F5A623] border-t-transparent rounded-full animate-spin" /></div>}
                ...
                return (
                  <button key={r.id} onClick={() => toggleSelect(r.id)}
                    className={`w-full flex items-center gap-3 rounded-2xl p-3 border transition-all text-left ${
                      sel ? "border-[#F5A623]/40 bg-[#F5A623]/5" : "border-white/8 bg-white/5 hover:border-white/10"
                    }`}>
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-all ${sel ? "bg-[#F5A623] border-[#F5A623]" : "border-white/15"}`}>
            ...
            <div className="px-5 pb-8 pt-3 border-t border-white/8 shrink-0 bg-[#000000]">
