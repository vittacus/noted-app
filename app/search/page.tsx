    ...
    <div className="page-enter">
      <div className="relative mb-5">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 rounded-2xl border border-white/10 bg-[#1a1a24] text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
          autoFocus
        />
        {loading && (
          ...
          {(["tracks", "albums"] as Tab[]).map((t) => (
            <button
              ...
              className={`flex-1 py-2 text-sm font-semibold rounded-xl transition-all capitalize ${
                tab === t ? "bg-[#1a1a24] text-blue-400 shadow-sm" : "text-slate-500 hover:text-slate-300"
              }`}
            >
      ...
      {tab === "tracks" && (
        <div className="space-y-2">
          {tracks.map((track) => (
            <div key={track.id} className="flex items-center gap-3 bg-[#1a1a24] rounded-2xl p-3 border border-white/5 hover:border-white/10 transition-colors">
              ...
              <button
                ...
                className="shrink-0 px-3 py-1.5 rounded-xl bg-blue-500/10 text-blue-400 text-xs font-semibold hover:bg-blue-500/20 transition-colors border border-blue-500/20"
              >
                Rate
              </button>
            </div>
        ...
        <div className="space-y-2">
          {albums.map((album) => (
            <Link
              key={album.id}
              href={`/album/${album.id}`}
              className="flex items-center gap-3 bg-[#1a1a24] rounded-2xl p-3 border border-white/5 hover:border-white/10 transition-colors block"
            >
              <div className="relative w-12 h-12 rounded-xl overflow-hidden bg-white/5 shrink-0">
                {album.images[0] ? (
