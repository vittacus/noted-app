    ...
    <div className="page-enter">
      ...
      <div className="relative mb-5">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 rounded-2xl border border-white/10 bg-[#111111] text-sm text-slate-100 placeholder-white/25 focus:outline-none focus:ring-2 focus:ring-[#F5A623]/50"
          autoFocus
        />
        {loading && (
          <Loader2 size={16} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#F5A623] animate-spin" />
        )}
      </div>
      ...
      {tab === "tracks" && (
        <div className="space-y-2">
          {tracks.map((track) => (
            <div key={track.id} className="flex items-center gap-3 bg-[#111111] rounded-2xl p-3 border border-white/8 hover:border-white/10 transition-colors">
              ...
              <button
                ...
                className={`shrink-0 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-colors ${
                  ratedMap[track.id]
                    ...
                    : "bg-[#F5A623]/10 text-[#F5A623] hover:bg-[#F5A623]/20 border-[#F5A623]/20"
                }`}
              >
